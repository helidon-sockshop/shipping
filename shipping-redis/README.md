...
This module implements [Redis backend](./src/main/java/io/helidon/examples/sockshop/shipping/redis/RedisShipmentRepository.java)
for the [Shipping Service](../README.md) using Redisson client.
...
```bash
$ docker run --rm --name shipping-db --network sockshop redis:5.0.7
``` 
...
```bash
$ docker run --network sockshop -p 7001:7001 ghcr.io/helidon-sockshop/shipping-redis
``` 
